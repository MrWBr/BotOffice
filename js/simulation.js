// ============================================================
// simulation.js — Versão Final (Fila VBA + Rotação + Neurônios + Recovery)
// ============================================================

window._BOT_SCHED = window._BOT_SCHED || {};

// CORPUS MARKOV PARA DIÁLOGOS DE MANUTENÇÃO / PCM
var MARKOV_CORPUS = [
...
function processarNeuroniosDecisao(bot) {
  ...
  var perf = (typeof CFG !== 'undefined' && CFG.PERFIL_PERSONALIDADES && CFG.PERFIL_PERSONALIDADES[bot.personality])
             ? CFG.PERFIL_PERSONALIDADES[bot.personality]
             : { modFadiga: 1.0, modEstresse: 1.0, limiarCafe: 0.60 };

  var iFadiga   = ((bot._fadiga || 0) / 100) * perf.modFadiga;
  var iEstresse = Math.min(1, ((bot.erros || 0) * 0.25)) * perf.modEstresse;
  var iEspera   = Math.min(1, tempoEsperaSeg / 15);
  var iRuido    = (Math.random() * 0.2) - 0.1;

  var nTrabalhar  = (iEspera * 0.7) - (iFadiga * 0.4) - (iEstresse * 0.3) + iRuido;
  var nPausaCopa  = (iFadiga * 0.6) + (iEstresse * 0.5) - (iEspera * 0.3) + iRuido;
  var nSocializar = (iEspera * 0.3) + (1 - iEstresse) * 0.3 + iRuido;

  return {
    dispararMacro: nTrabalhar > 0.40,
    irParaCopa:   nPausaCopa > perf.limiarCafe,
    falarChat:    nSocializar > 0.50
  };
}

// CALCULA HUMOR PARA O CRACHÁ
...
// RETORNO DA EXECUÇÃO DO VBA (AUTO-RECOVERY + REGRA DE DEMISSÃO)
window.receberRetornoVBA = function(botId, sucesso, mensagem) {
  ...
  if (sucesso) {
    ...
    // 🚨 REGRA DE DEMISSÃO AUTOMÁTICA POR ACÚMULO DE ERROS
    if (bot.erros >= 5) {
      bot.bub = 'Demitido! 📦🚪';
      bot.bubT = 300;
      bot.state = 'fired';
    ...
    // 🔄 AUTO-RECOVERY NORMAL (Sair do erro em 5 segundos)
    setTimeout(function() {
      if (bot.state === 'error') {
        bot.state = 'idle';
...
function sim(){
  ...
  BOTS.forEach(function(bot, i){
    ...
    if(bot.state === 'idle') {
      ...
      // ☕ PAUSA PARA O CAFÉ (Zera a fadiga acumulada)
      if (decisao.irParaCopa && !bot._emPausa) {
        ...
        // 🔄 ROTAÇÃO DE MACROS POR MAIOR ATRASO (ROUND-ROBIN DE PRIORIDADE)
        ...
        // 🚦 DISPARO CONTROLADO PELA FILA DO VBA
        if (macroPronta && decisao.dispararMacro) {
          if (!vbaOcupado) {
            vbaOcupado = true; // Trava o próximo bot no mesmo frame
